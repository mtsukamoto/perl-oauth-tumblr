# This Makefile is for the OAuth::Tumblr extension to perl.
#
...
#     ABSTRACT_FROM => q[lib/OAuth/Tumblr.pm]
#     AUTHOR => q[Makio Tsukamoto <tsukamoto@gmail.com>]
#     NAME => q[OAuth::Tumblr]
#     PL_FILES => {  }
#     PREREQ_PM => { version=>q[0], CGI=>q[0], Class::Accessor::Fast=>q[0], OAuth::Lite=>q[0], Class::ErrorHandler=>q[0], Test::More=>q[0], CGI::Session=>q[0], File::Slurp=>q[0] }
#     VERSION_FROM => q[lib/OAuth/Tumblr.pm]
#     clean => { FILES=>q[OAuth-Tumblr-*] }
...
TO_INST_PM = lib/OAuth/Tumblr.pm

PM_TO_BLIB = lib/OAuth/Tumblr.pm \
	blib\lib\OAuth\Tumblr.pm
...
DISTNAME = OAuth-Tumblr
DISTVNAME = OAuth-Tumblr-0.0.1
...
Tumblr.def: Makefile.PL
	$(PERLRUN) -MExtUtils::Mksymlists \
     -e "Mksymlists('NAME'=>\"OAuth::Tumblr\", 'DLBASE' => '$(BASEEXT)', 'DL_FUNCS' => {  }, 'FUNCLIST' => [], 'IMPORTS' => {  }, 'DL_VARS' => []);"
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) "name:               OAuth-Tumblr" >> META_new.yml
	$(NOECHO) $(ECHO) "version:            0.0.1" >> META_new.yml
	$(NOECHO) $(ECHO) "abstract:           OAuth for tumblr with OAuth::Lite::Consumer" >> META_new.yml
	$(NOECHO) $(ECHO) "author:" >> META_new.yml
	$(NOECHO) $(ECHO) "    - Makio Tsukamoto <tsukamoto@gmail.com>" >> META_new.yml
	...
	$(NOECHO) $(ECHO) "    Class::ErrorHandler:  0" >> META_new.yml
	$(NOECHO) $(ECHO) "    File::Slurp:          0" >> META_new.yml
	$(NOECHO) $(ECHO) "    OAuth::Lite:          0" >> META_new.yml
...
ppd :
	...
	$(NOECHO) $(ECHO) "    <ABSTRACT>OAuth for tumblr with OAuth::Lite::Consumer</ABSTRACT>" >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) "    <AUTHOR>Makio Tsukamoto &lt;tsukamoto@gmail.com&gt;</AUTHOR>" >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) "    <IMPLEMENTATION>" >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) "        <REQUIRE NAME=\"CGI::\" />" >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) "        <REQUIRE NAME=\"CGI::Session\" />" >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) "        <REQUIRE NAME=\"Class::Accessor::Fast\" />" >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) "        <REQUIRE NAME=\"Class::ErrorHandler\" />" >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) "        <REQUIRE NAME=\"File::Slurp\" />" >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) "        <REQUIRE NAME=\"OAuth::Lite\" />" >> $(DISTNAME).ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e "pm_to_blib({{@ARGV}, '$(INST_LIB)\auto', q[$(PM_FILTER)], '$(PERM_DIR)')" -- \
	  lib/OAuth/Tumblr.pm blib\lib\OAuth\Tumblr.pm 
